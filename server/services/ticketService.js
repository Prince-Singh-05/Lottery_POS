...
class TicketService {
	...
	static async generateTicketsForAllocation(allocation, ticketType) {
		...
		const expiryDate = new Date(
			new Date().getTime() +
				ticketType.expiryDuration * 60 * 24 * 60 * 1000
		);

		function biasedRandomNumber() {
			let biasThreshold;

			if (ticketType.name === "Daily Draws") biasThreshold = 0.99; // 99% chance
			if (ticketType.name === "Weekly Draws") biasThreshold = 0.95; // 95% chance
			if (ticketType.name === "Monthly Draws") biasThreshold = 0.9; // 90% chance

			if (Math.random() < biasThreshold) {
				// Generate a number less than 50
